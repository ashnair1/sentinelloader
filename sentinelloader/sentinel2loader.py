...
class Sentinel2Loader:
    def __init__(
        ...
        self.api = SentinelAPI(
            user, password, apiUrl, show_progressbars=showProgressbars
        )
        self.apiUrl = apiUrl if apiUrl[-1] == "/" else apiUrl + "/"
    ...
    def getProductBandTiles(self, geoPolygon, bandName, resolution, dateReference):
        ...
        logger.debug(
            "Getting contents. band=%s, resolution=%s, date=%s",
            bandName,
            resolution,
            dateReference,
        )
        ...
        bbox = rasterio.features.bounds(Polygon(geoPolygon))
        geoPolygon = [
            (bbox[0], bbox[3]),
            (bbox[0], bbox[1]),
            (bbox[2], bbox[1]),
        ...
        apicache_file = (
            self.dataPath
            + "/apiquery/Sentinel-2-S2MSI%s-%s-%s-%s-%s-%s.csv"
            % (
                productLevel,
                area_hash,
        ...
        products_df_sorted = products_df.sort_values(
            ["ingestiondate", "cloudcoverpercentage"], ascending=[False, False]
        )
        ...
        for index, pf in products_df_sorted.iterrows():
            ...
            if missing.area > 0:
                try:
                    if missing.buffer(0).intersects(footprint) == True:
                        missing = (missing.symmetric_difference(footprint)).difference(
                            footprint
                        )
        ...
        logger.debug(
            "Tiles selected for covering the entire desired area: %s", selectedTiles
        )
        ...
        for index, sp in products_df.loc[selectedTiles].iterrows():
            ...
            if self.cacheTilesData and os.path.isfile(meta_cache_file):
                ...
                if r.status_code != 200:
                    raise Exception(
                        "Could not get metadata info. status=%s" % r.status_code
                    )
                mcontents = r.content.decode("utf-8")
                saveFile(meta_cache_file, mcontents)

            rexp = (
                "<IMAGE_FILE>GRANULE\/([0-9A-Z_]+)\/IMG_DATA\/([0-9A-Z_]+_%s)<\/IMAGE_FILE>"
                % (bandName)
            )
            if productLevel == "2A":
                rexp = (
                    "<IMAGE_FILE>GRANULE\/([0-9A-Z_]+)\/IMG_DATA\/R%s\/([0-9A-Z_]+_%s_%s)<\/IMAGE_FILE>"
            ...
            if not self.cacheTilesData or not os.path.isfile(downloadFilename):
                ...
                if productLevel == "2A":
                    url = self.apiUrl + "odata/v1/Products('%s')/Nodes('%s.SAFE')/Nodes('GRANULE')/Nodes('%s')/Nodes('IMG_DATA')/Nodes('R%s')/Nodes('%s.jp2')/$value" % (
                        sp["uuid"],
                        sp["title"],
                        m.group(1),
                        resolutionDownload,
                ...
                if bandName == "TCI":
                    ...
                    ret = os.system(
                        "nearblack -o %s %s" % (tmp_tile_filetiff, tmp_tile_filejp2)
                    )
                    if ret != 0:
                        raise Exception(
                            "Error during 'nearblack' execution. code=%d" % ret
                        )

                    ret = os.system(
                        "gdal_translate %s %s" % (tmp_tile_filetiff, downloadFilename)
                    )
                    if ret != 0:
                        raise Exception(
                            "Error during 'gdal_translate' execution. code=%d" % ret
                        )

                    os.remove(tmp_tile_filetiff)
                else:
                    os.system(
                        "gdal_translate %s %s" % (tmp_tile_filejp2, downloadFilename)
                    )
    ...
    def cropRegion(self, geoPolygon, sourceGeoTiffs):
        ...
        s1 = crs_transform(bounds[0], bounds[1])
        s2 = crs_transform(bounds[2], bounds[3])
    ...
    def getRegionHistory(
        ...
        logger.info(
            "Getting region history for band %s from %s to %s at %s"
        ...
        while dateRef <= dateToObj:
            ...
            try:
                ...
                if bandOrIndexName in ["NDVI", "NDWI", "NDMI"]:
                    regionFile = self.getRegionIndex(
                        geoPolygon, bandOrIndexName, resolution, dateRefStr
                    )
                else:
                    regionFile = self.getRegionBand(
                        geoPolygon, bandOrIndexName, resolution, dateRefStr
                    )
                ...
                if pendingInterpolations > 0:
                    ...
                    logger.info(
                        "Calculating %s interpolated images" % pendingInterpolations
                    )
    ...
    def getRegionBand(self, geoPolygon, bandName, resolution, dateReference):
        regionTileFiles = self.getProductBandTiles(
            geoPolygon, bandName, resolution, dateReference
        )
    ...
    def getRegionIndex(self, geoPolygon, indexName, resolution, dateReference):
        if indexName == "NDVI":
            ...
            # get band 08
            nir, _, _ = self._getBandDataFloat(
            ...
            # get band 11
            b11, _, _ = self._getBandDataFloat(
                geoPolygon, "B11", resolution, dateReference
            )
            # calculate
            ...
            # get band 08
            swir, _, _ = self._getBandDataFloat(
                geoPolygon, "B10", resolution, dateReference
            )
            # calculate
            ...
            # get band 08
            b08, _, _ = self._getBandDataFloat(
                geoPolygon, self.nirBand, resolution, dateReference
            )
            # get band 02
            b02, _, _ = self._getBandDataFloat(
                geoPolygon, "B02", resolution, dateReference
            )
            # calculate
    ...
    def cleanupCache(self, filesNotUsedDays):
        os.system(
            "find %s -type f -name '*' -mtime +%s -exec rm {} \;"
